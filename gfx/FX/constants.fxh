...
static const float GB_CAM_MIN = 65.0;
static const float GB_CAM_MAX = 335.0;
static const float GB_CAM_MAX_FILLING_CLAMP = 1; // 0 to 1 value for clamping the fill when camera is at max distance
static const float GB_THRESHOLD = 1; // interpolation time
static const float GB_THRESHOLD2 = 1; // interpolation time
...
static const float GB_OPACITY_NEAR = 1; // Transparency when camera is near
static const float GB_OPACITY_FAR = 0.50;  // Transparency when camera is far
static const float BORDER_NIGHT_DESATURATION_MAX = 0.2; // how much border colors can get desaturated at night. 1.0f is full grey
static const float BORDER_FOW_REMOVAL_FACTOR = 0.8; // How much of the FOW that is removed from the borders. 1.0f is no FOW
static const float BORDER_LIGHT_REMOVAL_FACTOR = 0.99; // How much of the light calculations that are removed from the borders. 1.0f is no light
static const float GB_STRENGTH_CH1 = 0.5; // Opacity of bottom layer
...
static const float BORDER_MAP_TILE = 18000.0;
